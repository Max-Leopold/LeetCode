...
def walk(pos)
  ...
  return Set[pos] if HASH_INPUT[pos] == 9 
  DP[pos] = DIRS.map { 
    next_pos = pos + _1
    next_pos.imaginary.between?(0, INPUT.count - 1) && next_pos.real.between?(0, INPUT.first.count - 1) && HASH_INPUT[next_pos] - HASH_INPUT[pos] == 1 ? walk(next_pos) : Set[]
  }.reduce(:|)
end

pp HASH_INPUT.sum { |k, v| v == 0 ? walk(k).count : 0}