...
class Polymer
    ...
    def next_polymer(polymer_pair, polymer_table, limit, step)
    ...
    cache :next_polymer, -> (polymer_pair, polymer_table, limit, step) {(polymer_pair.to_s + step.to_s).hash}
end

polymer = Polymer.new
polymer_counts = initial_polymer[0..-2].zip(initial_polymer[1..-1]).map {|polymer_pair|
    polymer.next_polymer(polymer_pair, polymer_table, 40, 0)
}.inject {|agg, hash|
